...
int displayMenu() {
    ...
    tabText(3); printLine('=', 59);
    tabText(3); printf("| |       COURSE       | YEAR AND LEVEL | NO. OF STUDENTS |\n");
    tabText(3); printLine('=', 59);
...
void addSection(){
    ...
    newLine(5);
    tabText(5); printf("What is the course code?\n");
    tabText(5); scanf("%15s", section_course);

    tabText(5); printf("What is the year and section?\n");
    tabText(5); scanf("%10s", section_year);
...
void displayStudentRecord(int sectionIndex) {
    ...
    tabText(3); printLine('=', 65);
    tabText(3); printf("| |   STUDENT ID    |                 NAME                | SEX | \n");
    tabText(3); printLine('=', 65);
    for (int i = 0; i < num_students; i++) {
        tabText(3); printf("|%d| %-16s|  %-13s %-20s |  %c  |\n",
               i + 1,
...
void createGradesheet(int sectionIndex){
    ...
    if (num_gradesheets == 0) {
        ...
        for (int i = 0; i < num_gradesheets; i++) {
            tabText(3); printf("Enter scores for (%s) %s, %s:\n\n", gradesheets[i].student_id, gradesheets[i].last_name, gradesheets[i].first_name);

        // Read and validate Written Test Score
        tabText(4); printf("Written Test Score: ");
        while (scanf("%d", &gradesheets[i].written_tscore) != 1) {
            printf("Invalid input. Please enter a number: ");
            clearInputBuffer();
        }

        // Read and validate Performance Test Score
        tabText(4); printf("Performance Test Score: ");
        while (scanf("%d", &gradesheets[i].perform_tscore) != 1) {
            printf("Invalid input. Please enter a number: ");
            clearInputBuffer();
        }

        // Read and validate Exam Score
        tabText(4); printf("Exam Score: ");
        while (scanf("%d", &gradesheets[i].exam_score) != 1) {
            printf("Invalid input. Please enter a number: ");
            clearInputBuffer();
        }
        newLine(1);
    ...
    tabText(2); printLine('=',100);
    tabText(2); printf("| | %-20s | %-15s | %-15s | %-10s | %-9s | %-10s |\n", "Student ID", "Last Name", "First Name", "Total", "Average", "Status");
    tabText(2); printLine('=',100);
    for (int i = 0; i < num_gradesheets; i++) {
        tabText(2); printf("|%d| %-20s | %-15s | %-15s | %-10d | %-9d | %-10s |\n",
               i+1,
    ...
    int choice;
    tabText(2); printf("Would you like to download your gradesheet?\n");
    tabText(2); printf("[1] Yes\n");
    tabText(2); printf("[2] Return to Menu\n");
    tabText(2); printf("Enter your choice: ");
    ...
    while (!validChoice) {
        ...
        if (choice != 1 && choice != 2) {
            ...
            switch (choice) {
                case 1:
                    newLine(1);
                    tabText(2); printf("What would you like to name the file?\n");
                    tabText(2); scanf("%255s", buffer);
...
void downloadStudentRecord(int sectionIndex) {
    ...
    newLine(2);
    tabText(3); printf("What would you like to name the file?\n");
    tabText(3); scanf("%255s", buffer);
    snprintf(filename, sizeof(filename), "..\\..\\Proposal Program\\Your Files\\%s.csv", buffer);
...
void groupStudents(STUDENT students[], int numStudents) {
    ...
    int ans;
    newLine(1);
    tabText(2); printf("Enter the number of members per group: ");
    scanf("%d", &numMembers);
    ...
    newLine(1);
    tabText(2); printf("Would you like to create a .txt file for the Grouped Formed?\n");
    tabText(2); printf("[1] Yes\n");
    tabText(2); printf("[2] No, Return\n");
    tabText(2); printf("Enter your choice: ");
    scanf("%d", &ans);
...
void sortStudentsByName(STUDENT students[], int numStudents)
{
    ...
    newLine(1);
    tabText(2); printf("Would you like to create a CSV file for the sorted student names?\n");
    tabText(2); printf("[1] Yes\n");
    tabText(2); printf("[2] No\n");
    tabText(2); printf("Enter your choice: ");
    scanf("%d", &ans);
    ...
    if (ans == 1)
    {
        ...
        FILE* downloadSort = fopen(filename, "w");
        if (downloadSort == NULL) {
            printf("Error creating CSV File.\n");
            return;
        }

        fprintf(downloadSort, "STUDENT ID,LAST NAME,FIRST NAME\n");

        // Write the sorted information to the CSV file
        for (int i = 0; i < numStudents; i++)
        {
            fprintf(downloadSort, "%s, %s, %s\n",
                    students[i].student_id,
                    students[i].last_name,
                    students[i].first_name);
        }
